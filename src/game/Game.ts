...
export class Game {
  ...
  private setupWebSocket(): void {
    ...
    this.ws.onmessage = (event) => {
      ...
      switch (data.type) {
        ...
        case 'update':
          if (data.id !== this.player.id)
            this.updateRemotePlayer(data.id, data.x, data.y);
          break;
        case 'leave':
    ...
    const points = [
      ...
      },
      {
        x:
          ...
          GAME_CONSTANTS.COLLISION_MARGIN,
        y: spriteTop + GAME_CONSTANTS.COLLISION_MARGIN,
      },
      {
        x: spriteLeft + GAME_CONSTANTS.COLLISION_MARGIN,
  ...
  private drawCollisionBox(collisionBox: CollisionBox): void {
    ...
    this.debugGraphics.lineStyle(
      2,
      collisionBox.canWalk ? 0x00ff00 : 0xff0000,
      1
    );
  ...
  private centerGameContainer(): void {
    this.gameContainer.x =
      (window.innerWidth - GAME_CONSTANTS.WORLD_BOUNDS.width) / 2;
    this.gameContainer.y =
      (window.innerHeight - GAME_CONSTANTS.WORLD_BOUNDS.height) / 2;
  }
